...
@timing_decorator
def generate_from_image(image: types.Image,
    ...
    for n, generated_video in enumerate(operation.response.generated_videos):
        ...
        generated_video.video.save(f"video-image-{n}.mp4")  # save the video


def main():
    model = "veo-2.0-generate-001"
    image_gcs = "gs://easongy-onboarding-bucket/s.png"
    image = types.Image(
        gcs_uri=image_gcs,
        mime_type="image/png",
    )
    ...
    generate_from_image(image=image, config=config, model=model)
