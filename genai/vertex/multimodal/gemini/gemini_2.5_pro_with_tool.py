...
@timing_decorator
def generate_with_tool(prompt: str,
    ...
    for part in response.candidates[0].content.parts:
        ...
        if part.thought:
            ...
            print(f"Function to call: {part.function_call.name}")
...
def main():
    ...
    rank_tool = types.Tool(function_declarations=[rank_function])
    ...
    config = types.GenerateContentConfig(system_instruction=system_prompt,
                                         thinking_config=thinking_config,
                                         tools=[rank_tool, search_tool])
    generate_with_tool(prompt=prompt, config=config)
